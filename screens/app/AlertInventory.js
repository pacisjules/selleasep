...
const AlertInventory = ({ navigation }) => {
  ...
  //   const choosedDate=params.dating

  //For search products
  const [searchQuery, setSearchQuery] = useState("");

  const {
    all_inventoryAlert_error,
    all_inventoryAlert,
    all_inventoryAlert_isLoading,
...
const [company_Logo, setcompany_Logo] = useState(
  ...
  const onRefresh = () => {
    setRefreshing(true);
    dispatch(fetchInventoryAlertData(currentSpt));
  ...
  const Item = ({ name, quantity, status, percentage, last_updated }) => (
    <Center px="1">
      <View
        style={[
          styles.item,
          {
            borderColor: status==="Danger-Risk"?"#a30008":status==="High-Risk"?"#ed4700":status==="Medium-Risk"?"#ed9a00":status==="Low-Risk"?"#a6a43d":"#02a159",
            borderStyle: "solid",
            borderWidth: 2,
          },
        ]}
      >
        <View
          style={{
            ...
            padding: 6,
            borderRadius: 5,
            backgroundColor: status==="Danger-Risk"?"#a30008":status==="High-Risk"?"#ed4700":status==="Medium-Risk"?"#ed9a00":status==="Low-Risk"?"#a6a43d":"#02a159"
          }}
        >
          <Text
            style={{
        ...
        >
          <Text
            style={{
              fontSize: 10,
              color: status==="Danger-Risk"?"#a30008":status==="High-Risk"?"#ed4700":status==="Medium-Risk"?"#ed9a00":status==="Low-Risk"?"#a6a43d":"#02a159",
              textAlign: "right",
              fontFamily: "Poppins-Bold",
            }}
          >
  ...
  const createDynamicTable = () => {
    let table = "";
    for (let i = 0; i < all_inventoryAlert.length; i++) {
      const item = all_inventoryAlert[i];
      table += `
        <tr height="35px" >
          <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="left" width="150">${i+1}. ${item.name}</td>
          <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="left" width="100">${parseInt(item.percentage)} %</td>
          <td style="font-size: 12px; font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="left" width="100">${item.quantity}</td>
          <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: ${item.STATUS==="Danger-Risk"?"#a30008":item.STATUS==="High-Risk"?"#ed4700":item.STATUS==="Medium-Risk"?"#ed9a00":item.STATUS==="Low-Risk"?"#a6a43d":"#02a159"}; font-weight: bold;  vertical-align: top; padding: 0 0 7px;" align="left" width="100">${item.STATUS}</td>
          <td style="font-size: 9px; font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: bold;  vertical-align: top; padding: 0 0 7px;" align="right" width="150">${formatDateTime(item.last_updated)}</td>
        </tr>
          ...
          <View style={styles.container}>
            ...
            <TouchableOpacity
              ...
              <Input
                ...
                onChangeText={(e) => {
                  setSearchQuery(e);
                  dispatch(fetchInventoryAlertDataSearch(currentSpt, e));
                }}
                value={searchQuery}
                InputLeftElement={
                  <Icon
