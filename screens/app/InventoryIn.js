...
import {
    fetchInventoryHistoryData, fetchInventoryHistoryDataSearch
...
const InventoryIn = ({ navigation }) => {
  ...
  const { all_inventoryHistory_error,
    all_inventoryHistory,
    all_inventoryHistory_isLoading, } = useSelector(
    (state) => state.getallinventory
  );
...
const [reportData, setReportData] = useState(
  useSelector((state) => state.getallinventory.all_inventoryHistory)
);
...
const [location_Spt, setLocation_Spt] = useState(
  ...
  const onRefresh = () => {
    setRefreshing(true);
    dispatch(fetchInventoryHistoryData(currentSpt, choosedDate));
  ...
  const deleteSales = async () => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        setSelectedID(null);
        setIsOpen(false);
        dispatch(fetchInventoryHistoryData(currentSpt));
      })
      .catch((error) => {
        console.log(error);
  ...
  const createDynamicTable = () => {
    let table = "";
    for (let i = 0; i < all_inventoryHistory.length; i++) {
      const item = all_inventoryHistory[i];
      table += `
        <tr height="35px" >
          <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="left">${i+1}. ${item.emp_name}</td>
          <td style="font-size: 12px; font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="left">${item.action}</td>
          <td style="font-size: 9px; font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: bold;  vertical-align: top; padding: 0 0 7px;" align="right">${formatDateTime(item.timestamp)}</td>
        </tr>
        <tr height='20px'>
          ...
          <table width="600" border="0" cellpadding="0" cellspacing="0" align="center" class="fullPadding">
            ...
            <tr>
              ...
              <tr>
                ...
                <td style="font-size: 12px; font-family: 'Open Sans', sans-serif; color: #000; line-height: 22px; vertical-align: top; text-align:right; ">
                  <strong> ${all_inventoryHistory.length}</strong>
                </td>
              </tr>
  ...
  return (
    <SafeAreaView style={styles.containerer}>
      <NativeBaseProvider>
          ...
          <View style={styles.container}>
            ...
            <Center>
              ...
              <Input
                ...
                onChangeText={(e) => {
                  setSearchQuery(e);
                  dispatch(fetchInventoryHistoryDataSearch(currentSpt, choosedDate, e));
                }}
                value={searchQuery}
                InputLeftElement={
                  <Icon
        ...
        {all_inventoryHistory_isLoading ? (
          <View>
            <Center>
